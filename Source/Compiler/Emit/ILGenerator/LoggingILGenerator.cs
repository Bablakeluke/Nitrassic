	...
	internal class LoggingILGenerator : ILGenerator
	{
		...
		public override void StoreField(System.Reflection.FieldInfo field)
		{
			if (field == null)
				throw new ArgumentNullException("field");
			if (field.IsStatic == false)
				Log("stfld", field.FieldType+" "+field.Name+"/"+field.DeclaringType);
			else
				Log("stsfld", field.FieldType+" "+field.Name+"/"+field.DeclaringType);
		...
		{
			LogInstruction(instruction, Nitrassic.Library.String.Quote(stringConstant));
		}
