...
namespace Nitrassic.Compiler
{
	...
	internal class JSBinder : MethodBinder
	{
		...
		private static IEnumerable<BinderMethod> Convert(IEnumerable<JSBinderMethod> s)
		{
			
			List<BinderMethod> bm=new List<BinderMethod>();
			
			foreach(JSBinderMethod m in s)
			{
				bm.Add((BinderMethod)m);
			}
		...
		protected override void GenerateStub(ILGenerator generator, int argumentCount)
		{
			...
			//private static object SampleBinder(object thisObj, object[] arguments)
			//{
			...
			//	return thisObj.targetMethod(param1, param2, param3, param4);
			//}
			...
			foreach (var argument in binderMethod.GenerateArguments(generator, Math.Min(Math.Max(argumentCount, minArgumentCount), maxArgumentCount)))
			{
				switch (argument.Source)
				{
					...
					case BinderArgumentSource.ThisValue:
						...
						bool inheritsFromObjectInstance = typeof(Library.ObjectInstance).IsAssignableFrom(argument.Type);
						...
						else
						{
							...
							if (argument.Type != typeof(Library.ObjectInstance) && inheritsFromObjectInstance == true)
							{
								// If the value emitted is null it means that the "thisObj" is a type derived
								// from ObjectInstance (e.g. FunctionInstance) and the value provided is a different type
								// (e.g. Nitrassic.Library.Array).  In this case, throw an exception explaining that the function is
								// not generic.
			...
			else
			{
				...
				generator.Pop();
				/*if (binderMethod.ConvertNullToUndefined)
					EmitHelpers.EmitUndefined(generator);
				else*/
		...
		private static void EmitTypeConversion(ILGenerator il, Type fromType, Type toType)
		{
			...
			if (toType == typeof(int))
				EmitConversion.ToInteger(il, fromType);
			else
				EmitConversion.Convert(il, fromType, toType);
		}
